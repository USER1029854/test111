"""Cross-check the Arbitrum token set against the scraped SlowMist Hacked database.

Reuses /home/user/test111/data/slowmist_events.json directly -- it is a global,
chain-agnostic dump of the whole SlowMist Hacked database (not BSC-specific), so
no re-scrape is needed for this chain.
...
res = json.load(open(f"{DATA}/arb_abi_analysis.json"))
...
json.dump({"matched": out}, open(f"{DATA}/arb_hack_matches.json", "w"), indent=1)
