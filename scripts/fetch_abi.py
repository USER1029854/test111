"""Fetch verified ABI + source for the fixed 109-token set.

Not a re-discovery: the token list is read from bsc_new_projects.csv and never
...
OUT = "/tmp/claude-0/-home-user-test111/1040329a-6ac3-5d16-b840-114fc0760865/scratchpad/contracts"
    ...
    for t in range(tries):
        try:
            r = requests.get("https://api.etherscan.io/v2/api", timeout=40, headers=H,
                             params=dict(chainid=56, module="contract", action="getsourcecode",
...
json.dump({"meta": meta, "fails": fails}, open(f"{DATA}/contract_meta.json", "w"), indent=1)
