...
EXPLOIT below is populated by hand after reviewing base_hack_matches.json --
each entry must be grounded in a specific cited source; a ticker that merely
collides with an old entry is recorded as a collision, never as an exploit.
"""
import json, csv

DATA = "/home/user/test111/data"
a = json.load(open(f"{DATA}/base_abi_analysis.json"))
results, unverified = a["results"], a["unverified"]
clones = {k.lower(): v for k, v in json.load(open(f"{DATA}/base_minimal_proxies.json")).items()}

# Exploit / collision findings from base_hack_matches.json, filled in after review.
EXPLOIT = {}
...
dump(f"{DATA}/base_contract_logic.csv", kept)
dump(f"{DATA}/base_contract_logic_dropped.csv", dropped)
with open(f"{DATA}/base_contract_logic_unanalyzable.csv", "w", newline="") as f:
...
print("\nwrote base_contract_logic{,_dropped,_unanalyzable}.csv")
