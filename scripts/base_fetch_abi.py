"""Fetch verified ABI + source for the base_new_projects.csv set.

Not a re-discovery: the token list is read from base_new_projects.csv and never
changes. base_classify.py discarded the ABI/Proxy/Implementation fields, so they
are pulled once here. Proxies get their implementation fetched too, because a
proxy's own ABI is usually a near-empty shell and the real surface lives behind it.
"""
...
CHAINID = 8453
OUT = "/tmp/claude-0/-home-user-test111/958631bf-a002-5f41-98f2-bf8502fbeec3/scratchpad/contracts_base"
...
json.dump({"meta": meta, "fails": fails}, open(f"{DATA}/base_contract_meta.json", "w"), indent=1)
