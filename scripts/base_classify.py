...
Safety screening uses two providers, as on the BSC run:

  * honeypot.is  -- pass chainID=8453. Empirically works for old/blue-chip Base
                    tokens (verified against WETH before this run) but, per the
                    BSC precedent, is expected to under-cover very new tokens
                    because its indexer lags; real coverage numbers are printed
                    and recorded below rather than assumed.
  * GoPlus       -- https://api.gopluslabs.io/api/v1/token_security/8453, queried
                    one address at a time (batch mode only returns pre-cached
                    entries). Recovers tokens honeypot.is cannot see, and adds
                    holder/owner/LP signals.
...
# Verified on-chain (symbol()/name() via eth_call) before use -- see base_discover.py
# header. These are excluded from being treated as "candidate new projects" if they
# ever surface as a base_token; NOT the same as the stricter 4-asset set the brief
# names for the liquidity-verification USD conversion step (WETH/USDC/DAI/cbETH only).
MAJORS = {a.lower() for a in [
 "0x4200000000000000000000000000000000000006",  # WETH
 "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913",  # USDC
 "0x50c5725949A6F0c72E6C4a641F24049A917DB0Cb",  # DAI
 "0x2Ae3F1Ec7F1F5012CFEab0185bfc7aa3cf0DEc22",  # cbETH
 "0xd9aAEc86B65D86f6A7B5B1b0c42FFA531710b6CA",  # USDbC (legacy bridged USDC)
...
d = json.load(open(f"{DATA}/base_ds_qualifying.json"))
...
for i, (addr, e) in enumerate(sorted(by_token.items(), key=lambda kv: -kv[1]["best"]["_liq_usd"])):
    ...
    hp = get("https://api.honeypot.is/v2/IsHoneypot", {"address": tok, "chainID": CHAINID}, pause=0.3)
    ...
    # --- provider 3: Base source verification (Etherscan V2, chainid=8453; only module this key allows) ---
    ...
    rows.append({
        ...
        "vol6h_usd": round(float((p.get("volume") or {}).get("h6") or 0), 2),
...
json.dump(rows, open(f"{DATA}/base_classified.json", "w"), indent=1)
...
cols = ["name","symbol","token_address","pair_address","liquidity_usd","age_days","dex","quote",
        "quote_address","labels","vol24h_usd","vol6h_usd","txns24h","fdv_usd","market_cap_usd",
        "websites","socials","qualifying_pairs","dexscreener_url","screened","honeypot_is_state",
        "honeypot_is_result","honeypot_is_risk","goplus_state","goplus_is_honeypot","buy_tax_pct",
        "sell_tax_pct","holder_count","lp_holder_count","owner_percent","creator_percent","src_verified",
        "contract_name","risk_flags","memeish_name"]
with open(f"{DATA}/base_new_projects.csv","w",newline="") as f:
    w = csv.DictWriter(f, fieldnames=cols, extrasaction="ignore"); w.writeheader()
    for r in kept: w.writerow(r)
with open(f"{DATA}/base_excluded.csv","w",newline="") as f:
    w = csv.DictWriter(f, fieldnames=cols+["drop_reason"], extrasaction="ignore"); w.writeheader()
    for r in drp: w.writerow(r)
print("wrote base_new_projects.csv and base_excluded.csv")
