...
FINAL_LIQ, FINAL_AGE = 100_000.0, 90.0
...
for fn in ("arb_gt_pools.json", "arb_ds_search_pools.json"):
    path = f"{DATA}/{fn}"
...
def absorb(lst, src):
    for p in lst or []:
        if not isinstance(p, dict) or p.get("chainId") != "arbitrum": continue
...
for i in range(0, len(pair_addrs), 30):
    b = pair_addrs[i:i + 30]
    d = get(f"{DS}/latest/dex/pairs/arbitrum/" + ",".join(b))
...
for n, t in enumerate(tokens):
    d = get(f"{DS}/token-pairs/v1/arbitrum/{t}")
...
# ---- C. DEXScreener's own new-token feeds (Arbitrum slice) ------------------
prof = {}
for u in ["/token-profiles/latest/v1", "/token-boosts/latest/v1", "/token-boosts/top/v1"]:
    d = get(DS + u)
    for x in (d or []):
        if isinstance(x, dict) and x.get("chainId") == "arbitrum" and x.get("tokenAddress"):
            a = x["tokenAddress"].lower()
            prof.setdefault(a, {"links": x.get("links"), "description": x.get("description")})
print(f"DS profile/boost arbitrum tokens: {len(prof)}")
for t in prof:
    d = get(f"{DS}/token-pairs/v1/arbitrum/{t}")
...
json.dump({"qualifying": qual, "profiles": prof, "stats": stats, "discovery": srcinfo,
          ...
          open(f"{DATA}/arb_ds_qualifying.json", "w"), indent=1)
print("wrote arb_ds_qualifying.json | ds_calls=%d fail=%d" % (stats["ds_calls"], stats["ds_fail"]))
