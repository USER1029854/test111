...
a = json.load(open(f"{DATA}/arb_abi_analysis.json"))
results, unverified = a["results"], a["unverified"]
clones = {k.lower(): v for k, v in json.load(open(f"{DATA}/arb_minimal_proxies.json")).items()}

# Exploit / collision findings from arb_match_hacks.py review, each grounded in a
# specific cited SlowMist entry. A ticker that merely collides with an old entry
# is recorded as a collision, never presented as an exploit of this deployment.
EXPLOIT = {
}
...
for r in results:
    ...
    rows.append({**r,
        "age_band": "<=60d" if age <= 60 else ("61-90d" if age <= 90 else ">90d"),
...
cols = ["name","symbol","token_address","pair_address","liquidity_usd","age_days","age_band",
        "dex","quote","quote_address",
...
dump(f"{DATA}/arb_contract_logic.csv", kept)
dump(f"{DATA}/arb_contract_logic_dropped.csv", dropped)
with open(f"{DATA}/arb_contract_logic_unanalyzable.csv", "w", newline="") as f:
...
print("\nwrote arb_contract_logic{,_dropped,_unanalyzable}.csv")
