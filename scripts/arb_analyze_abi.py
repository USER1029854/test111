...
Same regex taxonomy as the BSC template (FUND, ADMIN_SET, ADMIN_MOVE, BRIDGE,
LIFECYCLE, MODIFIER, CTRL, UPGRADE, CONFIGISH) -- reused verbatim because it is
chain-agnostic Solidity-naming-convention logic, not a BSC-specific heuristic.

Two things ABI text alone cannot answer, handled the same way as the template:
  * ABIs do not encode access-control modifiers, so admin-only status is confirmed
    ...
    modifiers.
  * For proxies the proxy's own ABI is a shell, so the implementation ABI is
    merged in before classification, and EIP-1967 admin/impl slots + beacon slot
    are read from a public Arbitrum RPC.

Added beyond the BSC template: an explicit EIP-1167 minimal-proxy CLONE detector
via eth_getCode + the canonical clone bytecode pattern, run for every token in
scope (not just ones Etherscan's own `Proxy` flag caught) -- launchpad/points
factories on Arbitrum are exactly the kind of deployer that stamps out
minimal-proxy clones, and a clone's own "ABI" is a fund-logic-free shell.
Detected clones have their implementation source fetched on demand and merged
into the same classification pass, and the mapping is written to
arb_minimal_proxies.json (token -> implementation) for final_output to use for
clone-family / shared-codebase counting.
"""
import json, re, csv, os, requests, time

DATA = "/home/user/test111/data"
CDIR = "/tmp/claude-0/-home-user-test111/958631bf-a002-5f41-98f2-bf8502fbeec3/scratchpad/contracts"
...
# --- EIP-1967 slots + EIP-1167 clone detection via public Arbitrum RPC ---
...
# getRoleMember(DEFAULT_ADMIN_ROLE, 0) -- for proxies using AccessControl instead
# of Ownable, where owner() reverts and the real upgrade authority is the
# admin-role holder.
ROLE_MEMBER_CALL = "0x9010d07c" + "0" * 64 + "0" * 64
# canonical EIP-1167 minimal-proxy runtime bytecode, address in the middle
CLONE_RE = re.compile(r"^0x363d3d373d3d3d363d73([0-9a-f]{40})5af43d82803e903d91602b57fd5bf3", re.I)

def rpc_post(payload, tries=3):
    for url in RPCS:
...
def get_code(addr):
    return rpc_post({"jsonrpc":"2.0","id":1,"method":"eth_getCode","params":[addr, "latest"]})

def detect_clone(addr):
    code = get_code(addr)
    if not code or code in ("0x", "0x0"):
        return None
    m = CLONE_RE.match(code)
    if m:
        a = "0x" + m.group(1)
...
def raw_call(addr, data):
    v = rpc_post({"jsonrpc":"2.0","id":1,"method":"eth_call",
...
def eth_call(addr, fname):
    ...
    return raw_call(addr, sel)

def storage(addr, slot):
    v = rpc_post({"jsonrpc":"2.0","id":1,"method":"eth_getStorageAt","params":[addr, slot, "latest"]})
    if v and isinstance(v, str) and len(v) == 66:
        a = "0x" + v[-40:]
        return None if int(a, 16) == 0 else a
    return None

def fetch_src(addr, tries=4):
    for t in range(tries):
        try:
            r = requests.get("https://api.etherscan.io/v2/api", timeout=40,
                             headers={"User-Agent": "Mozilla/5.0"},
                             params=dict(chainid=CHAINID, module="contract", action="getsourcecode",
...
contract_meta_blob = json.load(open(f"{DATA}/arb_contract_meta.json"))
meta = contract_meta_blob["meta"]
fetch_fails = contract_meta_blob.get("fails", [])
rows = list(csv.DictReader(open(f"{DATA}/arb_new_projects.csv")))
by_addr = {r["token_address"].lower(): r for r in rows}
print(f"tokens in scope: {len(rows)}")

impl_src_cache = {}   # implementation address -> fetched source/ABI blob (avoid refetch)
clones = {}           # token addr -> implementation addr (EIP-1167 only)

results, unverified = [], []

# Tokens whose Etherscan source fetch failed at the network/retry level in
# arb_fetch_abi.py never got a `meta` entry at all, so without this they would
# silently vanish from the audit trail instead of being counted as
# not-analyzable. Recorded explicitly here so results+unverified == rows.
for fl in fetch_fails:
    b = by_addr.get((fl.get("token") or "").lower())
    if b:
        unverified.append({**b, "reason": f"source fetch failed: {fl.get('why','no response')}"})

for i, (addr, m) in enumerate(meta.items()):
    base = by_addr.get(addr)
    if not base: continue

    # --- universal EIP-1167 minimal-proxy clone check, run regardless of
    # Etherscan's own verified/proxy flags, since clone bytecode is often
    # either "verified" as an empty shell or not verified at all ---
    clone_impl = detect_clone(base["token_address"])
    if clone_impl:
        clones[addr] = clone_impl

    if not m["verified"] and not clone_impl:
        unverified.append({**base, "reason": "source not verified - ABI unavailable"})
        if i % 20 == 0: print(f"  {i}/{len(meta)} analyzed", flush=True)
        continue

    if os.path.exists(f"{CDIR}/{addr}.json"):
        blob = json.load(open(f"{CDIR}/{addr}.json"))
    else:
        blob = {"abi": "", "source": "", "impl_abi": "", "impl_source": ""}

    # merge in the clone implementation's ABI/source too, fetching on demand
    if clone_impl:
        ci = clone_impl.lower()
        if ci not in impl_src_cache:
            time.sleep(0.45)
            di = fetch_src(clone_impl)
            impl_src_cache[ci] = di
        di = impl_src_cache[ci]
        if di:
            ia = di.get("ABI") or ""
            if not ia.startswith("Contract source code not verified") and ia:
                # fold clone-implementation functions into the same merge slots
                # analyze_abi already reads ("abi","impl_abi") from
                if blob.get("impl_abi"):
                    blob["impl_abi_clone"] = ia
                else:
                    blob["impl_abi"] = ia
                blob["impl_source"] = (blob.get("impl_source") or "") + "\n" + (di.get("SourceCode") or "")

    fns = load_abi(blob)
    if blob.get("impl_abi_clone"):
        try:
            for it in json.loads(blob["impl_abi_clone"]):
                if it.get("type") == "function":
                    fns.append(it)
        except Exception:
            pass
    ...
    if not fns:
        unverified.append({**base, "reason": "verified flag set but ABI unparseable/empty"
                           if m["verified"] else "EIP-1167 clone, implementation source unavailable"})
        if i % 20 == 0: print(f"  {i}/{len(meta)} analyzed", flush=True)
        continue
    ...
    for f in fns:
        ...
        if ln in BOILER:
            continue
        # a view/pure function cannot move funds or change privilege -- constants such as
        # ALLOC_STAKING_REWARD would otherwise score as staking logic.
        if mut in ("view", "pure"):
            continue
    ...
    impl = m.get("implementation") or clone_impl
    admin_slot = None
    ...
    results.append({
        ...
        "quote_address": base.get("quote_address"),
        ...
        "is_proxy": m["is_proxy"] or custom_proxy,
        "proxy_kind": ("eip1167_clone" if clone_impl else
                       ("eip1967" if m["is_proxy"] else
                        ("custom_setImplementation" if custom_proxy else ""))),
        "implementation": impl or "",
        "minimal_proxy_clone": bool(clone_impl),
        "clone_implementation": clone_impl or "",
    ...
    if i % 20 == 0:
        print(f"  {i}/{len(meta)} analyzed", flush=True)

json.dump({"results": results, "unverified": unverified}, open(f"{DATA}/arb_abi_analysis.json","w"), indent=1)
json.dump(clones, open(f"{DATA}/arb_minimal_proxies.json", "w"), indent=1)
...
print(f"EIP-1167 clones found: {len(clones)}")
