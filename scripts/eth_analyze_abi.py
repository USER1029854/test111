...
Notes on method, because ABI alone is not sufficient for two of the asked questions:
  * ABIs do not encode access-control modifiers, so admin-only status is confirmed by
    locating the function declaration in the verified source and reading its modifiers.
  * For proxies the proxy's own ABI is a shell, so the implementation ABI is merged in
    before classification, and the EIP-1967 admin/impl slots are read from a public RPC.

Regex taxonomy (FUND, ADMIN_SET, ADMIN_MOVE, BRIDGE, LIFECYCLE, MODIFIER, CTRL,
UPGRADE, CONFIGISH) is reused verbatim from the BSC template (scripts/analyze_abi.py)
-- it already encodes hard-won lessons (view/pure exclusion, LayerZero OFT verbs,
migration/init, setImplementation) and the "custom logic vs plain token" line is
chain-agnostic Solidity, not BSC-specific.
"""
...
# ETH-specific addition vs. the BSC template: Chainlink CCIP (ccipReceive/ccipSend)
# is a well-documented bridge standard much more prevalent on Ethereum mainnet than
# BSC; without it, a CCIP receiver's fund-crediting entrypoint would be missed
# entirely by keyword match. Narrow, evidence-independent addition (exact selector
# names), not a taxonomy rewrite.
BRIDGE = re.compile(r"^(send|sendfrom|lzreceive|bridge|crosschain|teleport|relay|"
                    r"lzreceiveandrevert|lzreceivesimulate|ccipreceive|ccipsend)", re.I)
...
# --- EIP-1967 slots via public RPC ---
# The brief's suggested fallbacks (rpc.ankr.com/eth, cloudflare-eth.com) were tested
# live and both fail (ankr now requires an API key; cloudflare returns "Cannot
# fulfill request"), so they are replaced here with fallbacks verified working.
...
# getRoleMember(DEFAULT_ADMIN_ROLE, 0) -- these proxies use AccessControl, not Ownable,
...
def raw_call(addr, data):
    for url in RPCS:
        try:
            r = requests.post(url, timeout=15, json={"jsonrpc":"2.0","id":1,"method":"eth_call",
...
def storage(addr, slot):
    if not _rpc_ok[0]: return None
    for url in RPCS:
        try:
            r = requests.post(url, timeout=15, json={"jsonrpc":"2.0","id":1,
...
# --- EIP-1167 minimal-proxy clone detection via eth_getCode -----------------
# Runtime bytecode pattern: 363d3d373d3d3d363d73<20-byte impl>5af43d82803e903d91602b57fd5bf3
# (with minor prefix/suffix variants seen in the wild).
CLONE_RE = re.compile(r"363d3d373d3d3d363d73([0-9a-f]{40})5af43d82803e903d91602b57fd5bf3", re.I)
...
def detect_clone(addr):
    ...
    if not code or len(code) < 40:
        return None
    m = CLONE_RE.search(code)
    if m:
        return "0x" + m.group(1)
    return None

meta = json.load(open(f"{DATA}/eth_contract_meta.json"))["meta"]
...
results, unverified, clones = [], [], {}
for n, (addr, m) in enumerate(meta.items()):
    base = by_addr.get(addr)
    if not base: continue

    # EIP-1167 clone check runs regardless of verification status -- a clone of a
    # verified implementation is itself effectively "verified" via the implementation.
    clone_impl = detect_clone(base["token_address"])
    if clone_impl:
        clones[base["token_address"]] = clone_impl
    time.sleep(0.05)

    if not m["verified"] and not clone_impl:
        unverified.append({**base, "reason": "source not verified - ABI unavailable"})
        continue
    if not m["verified"] and clone_impl:
        # verify the implementation instead -- fetch its ABI/source live if we don't
        # already have it cached under the clone's own address.
        impl_path = f"{CDIR}/{clone_impl.lower()}.json"
        if not os.path.exists(impl_path):
            unverified.append({**base, "reason": f"EIP-1167 clone of {clone_impl}, implementation ABI not pre-fetched"})
            continue
        blob = json.load(open(impl_path))
    else:
    ...
    for f in fns:
        ...
        if mut in ("view", "pure"):
            continue
        if FUND.search(n_): fund.append(n_)
        if is_sig_gated(f): sig.append(n_)
        if ADMIN_SET.search(n_): admin_set.append(n_)
        if ADMIN_MOVE.search(n_): admin_move.append(n_)
        if BRIDGE.search(n_): bridge.append(n_)
        if LIFECYCLE.search(n_) or re.match(r"^init", ln): lifecycle.append(n_)
        if UPGRADE.search(n_): upgrade.append(n_)
        elif CONFIGISH.search(n_) and not ADMIN_SET.search(n_) and modifiers_of(src, n_):
            admin_cfg.append(n_)

    # admin-only confirmation from source modifiers
    gated = sorted({n_ for n_ in set(admin_set + admin_move + lifecycle + bridge + upgrade) if modifiers_of(src, n_)})
    ...
    results.append({
        ...
        "proxy_kind": ("eip1167_clone" if clone_impl else ("eip1967" if m["is_proxy"] else
                      ("custom_setImplementation" if custom_proxy else ""))),
        "implementation": clone_impl or impl or "",
    ...
    if n % 20 == 0:
        print(f"  analyzed {n}/{len(meta)}", flush=True)

json.dump({"results": results, "unverified": unverified, "clones": clones},
          open(f"{DATA}/eth_abi_analysis.json","w"), indent=1)
json.dump(clones, open(f"{DATA}/eth_minimal_proxies.json", "w"), indent=1)
...
print(f"EIP-1167 clones found (all tokens): {len(clones)}")
