"""Fetch verified ABI + source for the arb_new_projects.csv token set.

Not a re-discovery: the token list is read from arb_new_projects.csv and never
changes. arb_classify.py discarded the ABI/Proxy/Implementation fields, so they
are pulled once here. Proxies get their implementation fetched too, because a
proxy's own ABI is usually a near-empty shell and the real surface lives
behind it.
"""
...
json.dump({"meta": meta, "fails": fails}, open(f"{DATA}/arb_contract_meta.json", "w"), indent=1)
