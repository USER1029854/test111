...
CDIR = "/tmp/claude-0/-home-user-test111/1040329a-6ac3-5d16-b840-114fc0760865/scratchpad/contracts"

# Plain ERC-20/BEP-20 surface + ownership boilerplate. Nothing here is "fund handling".
...
# --- EIP-1967 slots via public RPC (BscScan proxy module is blocked on this key) ---
...
def eth_call(addr, fname):
    import hashlib
    try:
        from Crypto.Hash import keccak  # not guaranteed present
    except Exception:
        keccak = None
...
# precomputed keccak selectors (no keccak dependency needed for these two)
...
meta = json.load(open(f"{DATA}/contract_meta.json"))["meta"]
rows = list(csv.DictReader(open(f"{DATA}/bsc_new_projects.csv")))
...
for addr, m in meta.items():
    ...
    results.append({
        ...
        "proxy_kind": ("eip1967" if m["is_proxy"] else ("custom_setImplementation" if custom_proxy else "")),
...
json.dump({"results": results, "unverified": unverified}, open(f"{DATA}/abi_analysis.json","w"), indent=1)
