...
rows=[r for r in json.load(open(f"{DATA}/base_classified.json")) if not r["dropped"]]
random.seed(7)
sample=random.sample(rows, min(12,len(rows)))
NOW=time.time(); bad=0
print(f"verifying {len(sample)} of {len(rows)} rows against live DEXScreener\n")
for r in sample:
    d=requests.get(f"https://api.dexscreener.com/latest/dex/pairs/base/{r['pair_address']}",timeout=30,headers=H).json()
    ps=d.get("pairs") or []
    if not ps:
